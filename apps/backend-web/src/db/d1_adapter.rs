...
#[async_trait(?Send)]
impl DatabasePort for D1Database {
    // ============================================================
    // Book operations
    // ============================================================
    ...
    async fn add_book(&self, book: Book) -> Result<(), DbError> {
        ...
        let stmt = self
            .db
            .prepare("INSERT INTO books (isbn, title, series_title, authors, publisher, year, page_count, image_url, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)")
            .bind(&[
    ...
    async fn add_reading_log(&self, log: ReadingLog) -> Result<String, DbError> {
        let id = log.id.clone().unwrap_or_else(|| ulid::Ulid::new().to_string());

        let stmt = self
            .db
            .prepare("INSERT INTO reading_logs (id, isbn, created_at, session_duration_sec, page_start, page_end, rating) VALUES (?, ?, ?, ?, ?, ?, ?)")
            .bind(&[
                id.clone().into(),
    ...
    async fn add_laps(&self, reading_log: ReadingLog, laps: Vec<Lap>) -> Result<(), DbError> {
        // reading_logのIDを取得または生成
        let log_id = match reading_log.id {
            Some(id) => id,
            None => {
                // reading_logが新規の場合は先に作成
                self.add_reading_log(reading_log).await?
            }
        };
    ...
    async fn export_all(&self) -> Result<(Vec<Book>, Vec<ReadingLog>), DbError> {
        let books = self.find_books(QueryBuilder::new()).await?;
        let logs = self.find_reading_logs(QueryBuilder::new()).await?;
        Ok((books, logs))
    }
}
