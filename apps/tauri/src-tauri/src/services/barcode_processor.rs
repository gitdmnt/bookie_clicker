...
/// Application service for barcode detection from images
pub struct BarcodeProcessor;

impl BarcodeProcessor {
    /// Scan barcode from image bytes
    pub fn scan(image_bytes: &[u8]) -> Result<String, String> {
        let img = image::load_from_memory(image_bytes)
            .map_err(|e| format!("画像のデコードに失敗しました: {}", e))?;

        let img = Self::maybe_downscale(img);
        let gray = img.to_luma8();
        let binary = Self::binarize_otsu(&gray);
        let padded = Self::add_padding_white_gray(&binary, PADDING_RATIO);

        Self::detect_in_multi_lines(&padded)
    }
    ...
    fn detect_in_multi_lines(gray: &GrayImage) -> Result<String, String> {
        ...
        // Try scanning horizontal strips
